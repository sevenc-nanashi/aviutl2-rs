pub fn to_script_module_return_value(
    ...
    let fields = match ast.data {
        ...
        _ => {
            return Err(syn::Error::new_spanned(
                ast,
                "`ToScriptModuleReturnValue` can only be derived for structs with named fields",
            )
            .to_compile_error());
        }
    };
...
mod tests {
    ...
    fn test_to_script_module_return_value() {
        ...
        let output = super::to_script_module_return_value(input).unwrap();
