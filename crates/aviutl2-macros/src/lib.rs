...
/// # #[aviutl2_macros::filter_config_items]
/// # struct S {
/// #[checksection(name = "サンプルセクションチェック", default = true)]
/// section_bool_field: bool,
/// # }
/// ```
///
/// - `name`: セクションごとのチェックボックスの名前。省略した場合、フィールド名が使用されます。
/// - `default`: セクションごとのチェックボックスの初期値。
///
/// - 値の型は`bool`である必要があります。
///
/// ## `color`
...
///
/// #[hide]
/// #[track(name = "サンプル整数3", range = 0..=100, default = 50, step = 1.0)]
/// hide_field3: i32,
/// # }
/// ```
///
...
/// - 非表示条件は参照フィールド・演算子・値の組み合わせで指定します。
///   - 参照フィールドはフィールド名または`$filter`を使用して指定します。
///     - フィールドの種類は以下のいずれかのうち一つである必要があります：
///       - `check`。値がオンの場合は`1`、オフの場合は`0`になります。
///       - `checksection`。セクションごとに変更される場合は`2`、全体で固定の場合は`check`と同様です。
///       - `select`。EnumのVariantの値になります。
///       - `file`、`folder`。設定されている場合は`1`、設定されていない場合は`0`になります。
///     - `$filter`はフィルタオブジェクトかどうかを判定し、`0`または`1`になります。
///   - 演算子は`==`、`!=`、`>`、`<`、`>=`、`<=`のいずれかを使用します。
///     - `<=`、`>=`は内部的には`<`、`>`と`==`に展開されます。
...
/// 1つの関数に、同じシグネチャを持つ複数のメニュー属性を指定できます。
///
/// # Attributes
///
/// ### `import`
