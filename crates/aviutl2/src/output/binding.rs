...
pub trait OutputPlugin: Send + Sync + Sized {
    ...
    where
        Self: crate::output::__bridge::OutputSingleton,
    {
        <Self as crate::output::__bridge::OutputSingleton>::with_instance(f)
    }
    ...
    where
        Self: crate::output::__bridge::OutputSingleton,
    {
        <Self as crate::output::__bridge::OutputSingleton>::with_instance_mut(f)
    }
}
...
impl Drop for OutputInfo {
    fn drop(&mut self) {
        self.last_frame_id.store(usize::MAX, Ordering::SeqCst);
    }
}
