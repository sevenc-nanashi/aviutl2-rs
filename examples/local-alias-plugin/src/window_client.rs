...
const CLASS_NAME: &str = "LocalAliasWindowClient";
...
impl raw_window_handle::HasWindowHandle for WindowClient {
    fn window_handle(
        &self,
    ) -> Result<raw_window_handle::WindowHandle<'_>, raw_window_handle::HandleError> {
        let mut raw = raw_window_handle::Win32WindowHandle::new(
            std::num::NonZero::<isize>::new(self.hwnd.0 as isize)
                .ok_or(raw_window_handle::HandleError::Unavailable)?,
        );
        raw.hinstance = std::num::NonZero::<isize>::new(self.hinstance.0 as isize);
